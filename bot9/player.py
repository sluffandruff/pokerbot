...
class Player(Bot):
    ...
    def __init__(self):
        ...
        self.opp_raises_preflop = [0, 0, 0, 0]
        self.opp_raises_postflop = [0, 0, 0, 0]
    ...
    def get_actions(self, game_state, round_state, active):
        ...
        for i in range(NUM_BOARDS):
            ...
            ####################################################
            ### if we got called on the bluff, stop bluffing ###
            ####################################################
            if self.just_bluffed and i == self.board_bluffed:
                self.just_bluffed = False
                # print("HI", legal_actions[i])
                if (legal_actions[i] == {CheckAction} and street == self.street_on_bluff and not isinstance(board_state, TerminalState)) or (street != self.street_on_bluff and not isinstance(board_state, TerminalState)):
                    print("BLUFF CALLED", street, board_state)
                    # self.tightness = 0.85
            ...
            if AssignAction in legal_actions[i]:
                ...
                print(game_state.round_num, i, street, strength)
