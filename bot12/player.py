    ...
    def allocate_cards(self, my_cards):
        ...
        for i in range(5):
            for j in range(i + 1, 6):
                ...
                equities[hand] = eval7.py_hand_vs_range_monte_carlo(map(eval7.Card, hand), opp_range, [], 1500)
    ...
    def get_actions(self, game_state, round_state, active):
        ...
        for i in range(NUM_BOARDS):
            ...
            if AssignAction in legal_actions[i]:
                    ...
                    if street == 0:  # preflop
                        ...
                        if self.just_raised[i][0] and self.just_raised[i][1] != street:  # (raised, street raised on)
                            if street == 3 and not self.pf_call_raise_already[i]:
                                ...
                                hand_to_equity = eval7.py_all_hands_vs_range(self.prev_opp_est_of_range[i], self.prev_opp_est_of_range[i], self.rd_board[i], 125)
                ...
                if opp_action is not None:
                    ...
                    if game_state.round_num > 50:  # narrow it using percentage
                        if street == 0:
                            ...
                            top_percent = max(min(self.action_probs[RAISE], 1), 0)
                            hand_to_equity = eval7.py_all_hands_vs_range(self.opp_est_of_range[i], self.opp_est_of_range[i], self.rd_board[i], 125)
                            sorted_hands = [(k,1.0) for k, v in sorted(hand_to_equity.items(), key=lambda item: item[1], reverse=True)]
                            est_range_this_action = set(sorted_hands[:round(top_percent * len(sorted_hands))])
                ...
                if (self.times_called_bluff > 2 and self.times_bluff_succeeded / self.times_called_bluff < 10) or (self.times_called_bluff > 1 and self.times_bluff_succeeded / self.times_called_bluff < 5):
                ...
                if game_state.round_num > 50 and street > 0 and street != self.street_tracker and cont_cost == 0 and strength > max(self.action_probs[CALL_RAISE] + 0.05, 0.65):
