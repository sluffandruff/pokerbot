    ...
    def get_actions(self, game_state, round_state, active):
        ...
        for i in range(NUM_BOARDS):
            ...
            if AssignAction in legal_actions[i]:
                ...
                if strength < 0.5:
                    if active == 0 and legal_actions[i] == {CheckAction, RaiseAction} and all(self.equity[j] < 0.8 for j in range(3)) and random.random() < 0.4:
                ...
                if cont_cost > 0:
                    ...
                    if strength > max(pot_odds, pot/200):
                        if cont_cost >= 10:
                            if (street < 3 and strength > 0.6) or strength > 0.7:
