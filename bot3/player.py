...
class Player(Bot):
    ...
    def get_actions(self, game_state, round_state, active):
        ...
        #             self.equity[i] = eval7.py_hand_vs_range_monte_carlo(hand, self.opp_range[i], board, 10000)
            ...
            if AssignAction in legal_actions[i]:
                ...
                if street < 3:
                    if active == 0: # the opp is bb
                        if opp_pips[i] > 2*i: # opp has voluntarily put chips in (responding to our raise)
                            ...
                            if board_cont_cost > 0: # opp has raised in response to our raise
                                self.opp_pfr_round[i] = 1
                    else: # the opp is sb
                        if opp_pips[i] > i:
                            self.opp_vpip_round[i] = 1
                        if board_cont_cost > 0: # opp has raised
                            self.opp_pfr_round[i] = 1
                            
                if street == 3 and active == 0: # opp is bb, made it to the flop (neither of us folded)
                    if board_cont_cost > 0 and my_pips[i] == 0: # opp has made a cbet
                        self.opp_vpip_round[i] = 1
                ...
                if board_cont_cost > 0: #our opp raised!!! we must respond
                    ...
                    if strength >= pot_odds: #Positive Expected Value!! at least call!!
                        ...
                        net_cost += 0
                        if street < 3 and active == 0 and my_pips[i] == i: # I have folded the sb
                            self.opp_walks[i] += 1
