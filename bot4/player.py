            ...
            if AssignAction in legal_actions[i]:
                ...
                if street < 3:
                    if active == 0: # the opp is bb
                        if opp_pips[i] > 2*(i + 1): # opp has voluntarily put chips in (responding to our raise / raising after limp)
                    ...
                    if strength > 0.75:
                        ...
                        if cont_cost > 0:
                            ...
                            if strength > max(pot_odds, pot/200):
                                ...
                                my_actions[i] = FoldAction()
                                if street < 3 and active == 0 and my_pips[i] == i + 1: # I have folded the sb
                                    self.opp_walks[i] += 1
                        else:
                            my_actions[i] = CheckAction()
