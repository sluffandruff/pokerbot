...
class Player(Bot):
    ...
    def __init__(self):
        ...
        self.flops_seen = 0
        self.flop_r = 0
        self.flop_r_round = [0, 0, 0]
        self.turns_seen = 0
        self.turn_r = 0
        self.turn_r_round = [0, 0, 0]
        self.rivers_seen = 0
        self.river_r = 0
        self.river_r_round = [0, 0, 0]
    ...
    def handle_round_over(self, game_state, terminal_state, active):
        ...
        for i in range(NUM_BOARDS):
            ...
            self.flop_r += self.flop_r_round[i]
            self.flop_r_round[i] = 0
            self.turn_r += self.turn_r_round[i]
            self.turn_r_round[i] = 0
            self.river_r += self.river_r_round[i]
            self.river_r_round[i] = 0
        ...
        if round_num == NUM_ROUNDS:
            ...
            print(self.flops_seen, self.flop_r, self.turns_seen, self.turn_r, self.rivers_seen, self.river_r)
            ...
            if AssignAction in legal_actions[i]:
                ...
                if street == 3:
                    if street != self.street_tracker:
                        self.flops_seen += 1
                    if cont_cost > 0: # bb: opp has bet or check-raised, sb: opp has raised
                        self.flop_r_round[i] = 1
                        if self.flop_r_round[i] == 0:
                            recalc = True                
                elif street == 4:
                    if street != self.street_tracker:
                        self.turns_seen += 1
                    if cont_cost > 0: # bb: opp has bet or check-raised, sb: opp has raised
                        self.turn_r_round[i] = 1
                        if self.turn_r_round[i] == 0:
                            recalc = True
                elif street == 5:
                    if street != self.street_tracker:
                        self.rivers_seen += 1
                    if cont_cost > 0: # bb: opp has bet or check-raised, sb: opp has raised
                        self.river_r_round[i] = 1
                        if self.river_r_round[i] == 0:
                            recalc = True
                ...
                if street != self.street_tracker or recalc:
                    ...
                    if game_state.round_num > 50:
                        if street == 0:
                            ...
                            if (street == 3 and self.flop_r_round[i] == 1) or (street == 4 and self.turn_r_round[i] == 1) or (street == 5 and self.river_r_round[i] == 1):
                                rnge = [h for h in ALL_HANDS if h[0][0] not in board and h[0][1] not in board]
                                hand_to_equity = eval7.py_all_hands_vs_range(rnge, rnge, board, 200)
                                sorted_hands = [(k, 1.0) for k, v in sorted(hand_to_equity.items(), key=lambda item: item[1], reverse=True)]
                                if street == 3:
                                    percentage = self.flop_r / self.flops_seen
                                elif street == 4:
                                    percentage = self.turn_r / self.turns_seen
                                else:
                                    percentage = self.river_r / self.rivers_seen
                                # self.opp_range[i] = sorted_hands[:round(percentage * len(sorted_hands))]
                                opp_range = sorted_hands[:round(percentage * len(sorted_hands))]
                                prev_range = set(self.opp_range[i])
                                self.opp_range[i] = [h for h in opp_range if h in prev_range]
                                if street == 5:
                                    print(len(self.opp_range[i]))
