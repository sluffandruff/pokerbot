    ...
    def allocate_cards(self, my_cards):
        ...
        for i in range(5):
            for j in range(i + 1, 6):
                ...
                equities[hand] = eval7.py_hand_vs_range_monte_carlo(map(eval7.Card, hand), opp_range, [], 4000)
        ...
        for i in range(NUM_BOARDS):
            ...
            if AssignAction in legal_actions[i]:
                ...
                if street != self.street_tracker:                
                    ...
                    self.equity[i] = eval7.py_hand_vs_range_monte_carlo(hand, self.opp_range[i], board, 4000)
                ...
                if street < 3 and my_pips[i] == 1 and active == 0 and strength < 0.8:  # sb pre-flop 1st action, limp if s < 0.8
                    ...
                    if strength > 0.8:
                        ...
                        if cont_cost > 0:
                            ...
                            if strength > max(pot_odds, pot/200) and (cont_cost > 20 and strength > 0.7):
                                if (cont_cost <= my_stack - net_cost):
                                    my_actions[i] = CallAction()
                                    net_cost += cont_cost
                                else:
                                    my_action[i] = FoldAction()
                            else:
                                my_actions[i] = FoldAction()
